...
bool CustomerMode::authenticate(const std::string &userID, const std::string &password, ATM* atm) {
    ...
    if (incrementWrongAttempts() >= 3) exit(0);
    return false;
}
...
void CustomerMode::performCheckingOperations(ATM* atm) {
    while (true) {
        ...
        if (choice == 1) {
            ...
            Account* to = nullptr;
            for (auto a : atm->getAccounts()) if (a->getAccountID() == tid) to = a;
            UserInterface::showMessage("Enter amount to transfer:");
            double amt = UserInterface::getDoubleInput();
            if (to) transfer(atm, to, amt);
            else UserInterface::showMessage("Target account not found.");
        } else if (choice == 4) {
            ...
            break;
        } else UserInterface::showMessage("Invalid choice.");
